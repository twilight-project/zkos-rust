...
pub fn load_genesis_sets() -> Vec<RecordUtxo> {
    let read_data = fs::read("../utxo-in-memory\\src\\dbcurd\\genesis_sets.txt");
...
pub fn load_genesis_sets_test() -> Vec<RecordUtxo> {
    let read_data = fs::read("../utxo-in-memory\\src\\dbcurd\\test\\genesis_sets_test.txt");
    let mut record_utxo: Vec<RecordUtxo> = Vec::new();
    match read_data {
        Ok(data) => {
            record_utxo = bincode::deserialize(&data).unwrap();
...
// let mut file = std::fs::File::create("../utxo-in-memory\\src\\dbcurd\\genesis_sets.txt").unwrap();
...
pub fn set_genesis_sets() {
    ...
    let recordutxo = transaction::reference_tx::create_genesis_block(10000, 100, acc);
    let mut file =
        std::fs::File::create("../utxo-in-memory\\src\\dbcurd\\genesis_sets.txt").unwrap();
