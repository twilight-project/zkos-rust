...
pub fn process_transfer(transaction_id: String, transaction: String, block_height: u64) {
    ...
    let tx_id: [u8; 32] = hex::decode(transaction_id.clone()).expect("error decoding").try_into().expect("Vec<u8> must have exactly 32 elements");
...
pub fn process_trade_mint(
    ...
    let mut qq_account_bytes =
        hex::decode(qq_account).expect("Decoding failed");
