...
lazy_static! {
    pub static ref UTXO_STORAGE: Arc<Mutex<UTXOStorage<UtxoValue>>> =
        Arc::new(Mutex::new(UTXOStorage::<UtxoValue>::new()));
...
pub trait LocalDB<T> {
    ...
    ) -> Result<UTXO<T>, std::io::Error>;
    fn remove(
        &mut self,
        id: Vec<u8>,
        input_type: TxInputOutputType,
    ) -> Result<UTXO<T>, std::io::Error>;
    fn search_key(&mut self, id: &Vec<u8>, input_type: &TxInputOutputType) -> bool;
    fn get_utxo_by_id(
        &mut self,
        id: Vec<u8>,
        input_type: TxInputOutputType,
...
pub struct UTXOStorage<T> {
    pub coin_storage: HashMap<Vec<u8>, T>,
    pub memo_storage: HashMap<Vec<u8>, T>,
    pub state_storage: HashMap<Vec<u8>, T>,
