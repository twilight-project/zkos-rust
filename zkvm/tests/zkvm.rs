...
//     TxHeader, TxID, TxLog, VMError, Value, UnsignedTx
// };
...
// fn build_and_verify(program: Program, keys: &Vec<Scalar>) -> Result<UnsignedTx, VMError> {
//     //let (txlog, tx) = {
//         // Build tx
...
//     //useless predicates
//     let (preds, scalars) = generate_predicates(3);
//     let unsignedtx = build_and_verify(correct_program, &vec![scalars[1].clone()]).unwrap();
//     print!("{:?}", unsignedtx);
// }
