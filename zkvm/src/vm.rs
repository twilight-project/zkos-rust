...
use curve25519_dalek::digest::InvalidOutputSize;
use core::iter;
...
pub(crate) struct VMScript<'d, CS, D>
where
    CS: r1cs::RandomizableConstraintSystem,
    D: Delegate<CS>,
{
...
///SCRIPT EXECUTION
/// 
impl<'d, CS, D> VMScript<'d, CS, D>
where
    CS: r1cs::RandomizableConstraintSystem,
    D: Delegate<CS>,
{
    /// Instantiates a new VM instance.
    pub fn new(mintime: u64, maxtime: u64, ext: bool, run: D::RunType, delegate: &'d mut D, inputs: &'d[Input], outputs: &'d[Output]) -> Self {
        VMScript {
            mintime_ms: mintime,
            maxtime_ms: maxtime,
            extension: ext,
    ...
    /// Runs through the entire program and nested programs until completion.
    pub fn run(mut self) -> Result<(CheckedFee), VMError> {
        loop {
            if !self.step()? {
                break;
            }
...
*/
    ...
    fn inputcoin(&mut self, i: usize) -> Result<(), VMError>{
        ...
        let check = input.in_type.is_coin(); 
        if check {
            let encrypt = input.input.get_encryption().ok_or(VMError::InvalidInputCoin)?;
            let address = input.input.get_owner().ok_or(VMError::InvalidInputCoin)?;

            let c = OutputCoin{encrypt: encrypt.clone(), address: address.clone()};
            self.push_item(c);
            Ok(())
        }else{
            Err(VMError::InvalidInputCoin)
    ...
    fn outputcoin(&mut self, i: usize) -> Result<(), VMError>{
        ...
        let check = out.out_type.is_coin(); 
        if check == true{
            let encrypt = out.output.get_encrypt().ok_or(VMError::InvalidOutputCoin)?;
            let address = out.output.get_address().ok_or(VMError::InvalidOutputCoin)?;
